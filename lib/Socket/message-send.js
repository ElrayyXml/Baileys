    ...
    const relayMessage = async (jid, message, { messageId: msgId, participant, additionalAttributes, additionalNodes, useUserDevicesCache, useCachedGroupMetadata, statusJidList }) => {
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isPrivate) {
                const hasMeta = stanza.content.some(n => n.tag === 'meta');
                if (!hasMeta) {
                    stanza.content.push({
                        tag: 'meta',
                        attrs: {},
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'album' in content && content.album) {
                ...
                await relayMessage(jid, fullMsg.message, { messageId: fullMsg.key.id, useCachedGroupMetadata: options.useCachedGroupMetadata, additionalAttributes, additionalNodes: options.additionalNodes || additionalNodes, statusJidList: options.statusJidList });
