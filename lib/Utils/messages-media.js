...
const getRawMediaUploadData = async (media, mediaType, logger) => {
    ...
    const filePath = path_1.join(os_1.tmpdir(), mediaType + generics_1.generateMessageIDV2());
...
async function generateThumbnail(file, mediaType, options) {
    ...
    if (mediaType === 'image') {
        ...
        const imgFilename = path_1.join(getTmpFilesDirectory(), generics_1.generateMessageIDV2() + '.jpg');
        try {
