...
const generateWAMessageContent = async (message, options) => {
    ...
    if ('text' in message) {
        ...
        /***
        * Add thumbnailLink Modified
        */
        if ('thumbnailLink' in message) {
            const link = message.thumbnailLink;
            const triggerUrl = (link.matchedText || link.thumbnailUrl || link.sourceUrl || "").trim();

            const processWithSharp = async (input, isFavicon = false) => {
                if (!input) return null;
                let buffer;

                try {
                    if (typeof input === 'string' && input.startsWith('http')) {
                        const res = await axios_1.get(input, { responseType: 'arraybuffer' });
                        buffer = Buffer.from(res.data);
                    } else if (typeof input === 'string' && fs_1.existsSync(input)) {
                        buffer = fs_1.readFileSync(input);
                    ...
                    let pipeline = sharp_1(buffer);
                    if (isFavicon) {
                        pipeline = pipeline.resize(48, 48, { fit: 'cover' });
                    }
                    ...
                    const metadata = await sharp_1(processedBuffer).metadata();
                    const prepared = await (0, exports.prepareWAMessageMedia)(
                    ...
                    options.logger?.error({ e }, 'Sharp processing failed in thumbnailLink');
                    return null;
                }
            };

            const [mainThumb, favThumb] = await Promise.all([
                processWithSharp(link.thumbnail || link.thumbnailUrl, false),
                processWithSharp(link.favicon || link.faviconUrl, true)
            ]);

            extContent.title = link.title || '';
            extContent.description = link.description || link.body || '';
            extContent.matchedText = triggerUrl;
            extContent.previewType = 0;
            extContent.renderLargerThumbnail = typeof link.renderLargerThumbnail !== 'undefined' 
                ? !!link.renderLargerThumbnail 
                : true;

            if (mainThumb) {
                extContent.thumbnailDirectPath = mainThumb.directPath;
                extContent.mediaKey = mainThumb.mediaKey;
                extContent.mediaKeyTimestamp = mainThumb.mediaKeyTimestamp;
                extContent.thumbnailSha256 = mainThumb.fileSha256;
                extContent.thumbnailEncSha256 = mainThumb.fileEncSha256;
                extContent.thumbnailWidth = mainThumb.width;
                extContent.thumbnailHeight = mainThumb.height;
                extContent.jpegThumbnail = mainThumb.jpegThumbnail;
            }

            if (favThumb) {
                extContent.faviconMMSMetadata = {
                    thumbnailDirectPath: favThumb.directPath,
                    mediaKey: favThumb.mediaKey,
                    mediaKeyTimestamp: favThumb.mediaKeyTimestamp,
                    thumbnailSha256: favThumb.fileSha256,
                    thumbnailEncSha256: favThumb.fileEncSha256,
                    thumbnailWidth: favThumb.width,
                    thumbnailHeight: favThumb.height
                };
            }
        }
        else {
    ...
    if ('interactiveButtons' in message && !!message.interactiveButtons) {
        /*
        * Custom interactiveButtons 
        */
        const button = message.buttons || message.interactiveButtons;
        const params = message.messageParamsJson || message.paramsJson || '';

        const interactiveMessage = {
            nativeFlowMessage: Types_1.WAProto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                 buttons: button,
                 messageParamsJson: typeof params === 'object' ? JSON.stringify(params) : params
              })
         };
        ...
        const headerSrc = message.header || null;
        const hasTitle = ('title' in message && !!message.title) || !!headerSrc?.title;
        const hasLocation = ('location' in message && !!message.location) || !!headerSrc?.location;

        if (headerSrc || hasTitle || hasLocation || Object.keys(m).length > 0) {
            interactiveMessage.header = {
                title: headerSrc?.title || message.title || null,
                subtitle: headerSrc?.subtitle || message.subtitle || null,
                hasMediaAttachment: headerSrc
                    ? !!headerSrc.hasMediaAttachment
                    : ((_k = message?.media) !== null && _k !== void 0 ? _k : false),
                };

        if (hasLocation) {
                const loc = headerSrc?.location || message.location;
                let thumb = loc.thumbnail || loc.jpegThumbnail;

                if (thumb) {
                    if (typeof thumb === 'string' && /^https?:\/\//.test(thumb)) {
                        const res = await axios_1.default.get(thumb, { responseType: 'arraybuffer' });
                        thumb = Buffer.from(res.data);
                    } else if (typeof thumb === 'string' && fs_1.existsSync(thumb)) {
                        thumb = fs_1.readFileSync(thumb);
                    }
                    if (Buffer.isBuffer(thumb)) {
                        thumb = await sharp_1(thumb)
                            .resize(300, 300, { fit: 'cover' })
                            .toBuffer();
                    }
                }

                interactiveMessage.header.locationMessage = Types_1.WAProto.Message.LocationMessage.fromObject({
