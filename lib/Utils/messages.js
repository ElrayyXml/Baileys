    ...
    if ('text' in message) {
        ...
        */
        if ('thumbnailLink' in message) {
            ...
            const processInternalMedia = async (input) => {
                if (!input) return null;
                try {
                    if (typeof input === 'string' || Buffer.isBuffer(input) || (typeof input === 'object' && 'url' in input)) {
                        const prepared = await (0, exports.prepareWAMessageMedia)(
                            { image: (typeof input === 'string' || Buffer.isBuffer(input)) ? input : input.url },
            ...
            const [mainThumb, favThumb] = await Promise.all([
                processInternalMedia(link.thumbnail),
                processInternalMedia(link.favicon)
            ]);

            extContent.title = link.title || '';
            extContent.description = link.description || '';
            extContent.matchedText = link.matchedText || (0, exports.extractUrlFromText)(message.text) || '';
            extContent.previewType = link.previewType || 0;
            ...
            if (mainThumb) {
                ...
                extContent.thumbnailWidth = link.width || mainThumb.width;
                extContent.thumbnailHeight = link.height || mainThumb.height;
                extContent.jpegThumbnail = link.jpegThumbnail || mainThumb.jpegThumbnail;
            }
        ...
        */
        ...
        if (message.header) {
            interactiveMessage.header = {
                title: message.header.title || null,
                subtitle: message.header.subtitle || null,
                hasMediaAttachment: !!message.header.hasMediaAttachment
            };
        if (message.header.location) {
                const loc = message.header.location;
                let thumb = loc.thumbnail || loc.jpegThumbnail || message.jpegThumbnail;

            if (thumb) {
                if (typeof thumb === 'string' && fs_1.existsSync(thumb)) {
